...
class SWMedianProbVL : public IModule{
  ...
  public:
	...
	flag("both"){
		...
		MAP_RX_PORT(2,sorted_list);
		MAP_TX_PORT(1,probabilities);
...
void SWMedianProbVL::compute(void){
	...
	//changing RRR
	MessageQueue::Message* im1 = READ_PORT(2);
	...
	LOG_PTAG(Debug) << "Sorted list size: " << Size << endl;
    LOG_PTAG(Debug)<< "Allocating median data structure ... " << flush;

//changing RRR
	MessageQueue::Message* im2 = READ_PORT(1);
    ...
    exact.write_prob(nNoDupl, (vector<InType>)sortedList, *outSWdata);
