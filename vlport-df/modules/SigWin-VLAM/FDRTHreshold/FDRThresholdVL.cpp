...
//	logLevel("log_level",DEFAULTLOGLEVEL),
   ...
   // rx_ports[1] = new MessageQueue("x_axis");//(char*)flag.get_in_name(1));
   ...
   // tx_ports[1] = new MessageQueue("fdr_threshold");
	...
	MAP_RX_PORT(2,probabilities);
	MAP_RX_PORT(3,frequencies);
	MAP_TX_PORT(1,fdr_threshold);
...
void FDRThresholdVL::compute_FDR(){
    ...
    test_header(*inStreamList, flag.get_in_name(1),flag.get_in_flag(1));
...
//RRR
	//MessageQueue::Message* im2 = rx_ports[3]->Read();
	MessageQueue::Message* im2 = READ_PORT(3);
	inStreamFreq->write( (char*)im2->mpData, im2->mDataLength);
	SET_RX_PORT_STATE(3,1);
	SIGNAL_RX_PORT(3);

    test_header(*inStreamFreq, flag.get_in_name(3),flag.get_in_flag(3));
    
    LOG_PTAG(Debug) << "Testing inStreamProb ..." << flush;

	//MessageQueue::Message* im3 = rx_ports[2]->Read();
    MessageQueue::Message* im3 = READ_PORT(2);
