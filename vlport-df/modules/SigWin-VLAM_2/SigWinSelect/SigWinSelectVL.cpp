...
void SigWinSelectVL::compute(void){
	...
	MessageQueue::Message* im3 = rx_ports[1]->Read();
	inFDRThresh->write( (char*)im3->mpData, im3->mDataLength);

    //------------------------------------------------------------
    test_header(*inSWParameters, flag.get_in_name(3),flag.get_in_flag(3));
    test_header(*inSWData,flag.get_in_name(2),flag.get_in_flag(2));
    test_header(*inFDRThresh, flag.get_in_name(1),flag.get_in_flag(1));
