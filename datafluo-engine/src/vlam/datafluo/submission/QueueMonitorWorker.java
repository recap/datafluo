...
import vlam.datafluo.reactor.ServerEntry;
...
class QueueMonitorWorker implements Runnable {
	...
	public synchronized void run() {
		try {
			...
			GlobalConfiguration.logging.log("QUEUELOAD: " + data + " " + GlobalConfiguration.logging.getTime());
	...
	private synchronized ServerEntry getServerByName(String serverName){
		for(Iterator itr = GlobalConfiguration.getServers().iterator(); itr.hasNext();){
			ServerEntry se = (ServerEntry)itr.next();
	...
	private synchronized void updateSubmitterStats(String data) {
		...
		/*se = (ServerEntry)getServerByName(shortHostname);
		if(se == null){
			synchronized (GlobalConfiguration.serverSync) {
				se = new ServerEntry(hostname,GlobalConfiguration.protocol,1,hostname+"?"
		...
		GlobalConfiguration.logging.debug("Consumed " + is.getName() + " cost: " + is.getConsumedCost(),12);
